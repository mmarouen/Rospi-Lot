...
int main (int argc, char **argv)
{
	...
	ros::Subscriber lanes_sub = n.subscribe("/control/pid/cmd_vel", 1,yaw_callback);	
    ...
    while (time(NULL)-current<duration)
    {
        ...
        if(twist.angular.z != NULL){
            pidControl(twist.angular.z);

            i2cpwm_board::Servo throttle;
            throttle.servo =1;
            throttle.value = twist.linear.x*0.5*range+center_val;
            i2cpwm_board::Servo steering;
            steering.servo =2;
            steering.value = current_alpha*0.5*range+center_val;
